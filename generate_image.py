...
return "cuda"
# On Apple Silicon, MPS may be available
...
# For faster load on limited memory, consider torch_dtype=torch.float16 and use cpu/offload
pipe = StableDiffusionPipeline.from_pretrained(args.model, torch_dtype=torch.float16 if device=="cuda" else torch.float32)
pipe = pipe.to(device)


print("Generating image...")
image = pipe(args.prompt, num_inference_steps=args.num_inference_steps, guidance_scale=args.guidance_scale, height=args.height, width=args.width).images[0]
image.save(args.out)
